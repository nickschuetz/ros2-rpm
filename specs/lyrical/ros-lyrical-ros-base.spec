...
Version:        0.13.1
Release:        1%{?dist}
Summary:        ROS 2 Lyrical default install, ros-core plus tf2 and common message types
...
Source0:        ros-lyrical-ros-base-%{version}.tar.gz

BuildArch:      noarch

# Pulls in the full ros_core (rclcpp, RMW + Fast DDS, message foundations)
Requires:       ros-lyrical-ros-core

# Workspace setup files (this is what makes `source /opt/ros/lyrical/setup.bash` work).
Requires:       ros-lyrical-ros-workspace
Requires:       ros-lyrical-ros-environment
...
Requires:       ros-lyrical-trajectory-msgs
...
Requires:       ros-lyrical-ackermann-msgs
Requires:       ros-lyrical-vision-msgs
Requires:       ros-lyrical-control-msgs

%description
Default install for ROS 2 Lyrical users. Pulls in ros-core (rclcpp, RMW/Fast DDS,
...
Phase 2 dev-sandbox metapackage `ros-lyrical-ros-desktop` is a separate
...
echo "ros-lyrical-ros-base %{version} (metapackage)" > %{buildroot}%{install_prefix}/.ros-base-version
...
- Add ros-workspace, ros-environment, rclpy to the dependency set so a
  fresh `dnf install ros-lyrical-ros-base` results in a usable Python +
  C++ ROS environment with /opt/ros/lyrical/setup.bash present.
