...
Patches are per-distro: this directory holds patches for the distro named by its
parent path (`specs/<distro>/patches/`). The same upstream package can need a
different patch on Jazzy than on Lyrical, so keep them separate.
...
```
specs/<distro>/patches/<package>/NNNN-short-kebab-name.patch
```

- `<distro>`: the ROS distro (`jazzy`, `lyrical`, ...).
- `<package>`: the upstream package name (no `ros-<distro>-` prefix).
- `NNNN`: 4-digit sequence, starting at `0001`. Sequence is per-package.
- `short-kebab-name`: 2-4 words describing the intent.

Example: `specs/lyrical/patches/rviz_ogre_vendor/0001-cmake-policy-minimum-3.20.patch`
...
3. Delete the patch file from `specs/<distro>/patches/<package>/`.
4. Re-pin `distros/<distro>/packages.yaml` to the upstream merge tag (or the
   next bloom-release that contains it).
