...
| [`build.yml`](../.github/workflows/build.yml) | push, PR | Matrix-build dry-run on 6 chroot/arch pairs (Fedora 44 + rawhide + Stream 10 × 2 arches). The COPR chroot matrix is 8 (Fedora 45 added per [ADR 0013](adr/0013-add-fedora-45-chroots.md)); Fedora 45 has no CI container image until it ships (GA 2026-10-20), so it is exercised on COPR only. |
